    ...
    <div class="bg-white rounded-xl shadow-sm p-6">
        <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="md:col-span-2 flex items-end space-x-2">
                ...
                <a href="{{ route('reports.profit') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300">
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <div>
                        <p class="text-sm text-gray-500">Transaksi Pendapatan</p>
                        <p class="text-xl font-bold text-gray-800">{{ $totalIncomeTransactions }}</p>
                    </div>
                </div>
                <div class="text-right">
                    <p class="text-sm text-gray-500">Rata-rata</p>
                    <p class="font-semibold text-green-600">Rp {{ number_format($averageIncomePerTransaction, 0, ',', '.') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <div>
                        <p class="text-sm text-gray-500">Transaksi Pengeluaran</p>
                        <p class="text-xl font-bold text-gray-800">{{ $totalExpenseTransactions }}</p>
                    </div>
                </div>
                <div class="text-right">
                    <p class="text-sm text-gray-500">Rata-rata</p>
                    <p class="font-semibold text-red-600">Rp {{ number_format($averageExpensePerTransaction, 0, ',', '.') }}</p>
                </div>
            </div>
        </div>
    </div>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @php
                            ...
                            for ($i = 0; $i < count($dates); $i++) {
                                $dailyData[] = [
                                    ...
                                    'income_transactions' => $incomeTransactions[$i] ?? 0,
                                    'expense_transactions' => $expenseTransactions[$i] ?? 0,
                                ];
                            }
                        @endphp
                        ...
                        @foreach ($dailyData as $daily)
                            @php
                                ...
                                $statusClass = $daily['profit'] >= 0 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800';
                            @endphp
                            <tr>
                                <td class="px-4 py-3">
                                    <div class="font-medium text-gray-900">{{ $daily['date'] }}</div>
                                    <div class="text-xs text-gray-500">
                                        {{ $daily['income_transactions'] }} transaksi | {{ $daily['expense_transactions'] }} restock
                                    </div>
                                </td>
                                <td class="px-4 py-3">
                                    <div class="text-gray-900">Rp {{ number_format($daily['income'], 0, ',', '.') }}</div>
                                </td>
                                <td class="px-4 py-3">
                                    <div class="text-gray-900">Rp {{ number_format($daily['expense'], 0, ',', '.') }}</div>
                                </td>
                                <td class="px-4 py-3">
                                    <div class="font-bold {{ $profitClass }}">Rp {{ number_format($daily['profit'], 0, ',', '.') }}</div>
                                </td>
                                <td class="px-4 py-3">
                                    <div class="font-medium {{ $marginClass }}">{{ number_format($daily['margin'], 2) }}%</div>
                                </td>
                                <td class="px-4 py-3">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <div class="flex flex-wrap items-center justify-between mb-1 gap-1">
                            ...
                            <div class="flex items-center gap-2 flex-wrap">
                                ...
                                @if ($expensePercentageRaw > 100)
                                    <span class="text-xs bg-red-100 text-red-800 px-2 py-0.5 rounded-full whitespace-nowrap">
                        ...
                        <div class="w-full bg-gray-200 rounded-full h-2.5">
                            ...
                            <div class="bg-red-600 h-2.5 rounded-full" style="width: {{ $expenseDisplayPercentage }}%;"></div>
                        </div>
                        <div class="flex flex-wrap items-center justify-between mt-1 gap-1">
                            ...
                            <span class="text-sm {{ $isLoss ? 'text-red-600 font-semibold' : 'text-gray-500' }} whitespace-nowrap">
                    ...
                    <div>
                        ...
                        @if ($profitPercentageRaw >= 0)
                            <!-- Profit Positif -->
                            <div class="w-full bg-gray-200 rounded-full h-2.5">
                                <div class="{{ $profitBarClass }} h-2.5 rounded-full" style="width: {{ $profitDisplayPercentage }}%"></div>
                            </div>
                            <div class="flex justify-end mt-1">
                    ...
                    @if ($isLoss)
                        <div class="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg">
                            <div class="flex items-start">
                                <i data-lucide="alert-triangle" class="w-5 h-5 text-red-600 mr-2 shrink-0 mt-0.5"></i>
                                <div class="min-w-0 flex-1">
                                    ...
                                    <div class="text-xs text-red-700 mt-1 space-y-0.5">
                                        <div class="flex justify-between gap-2">
                                            ...
                                            <span class="font-medium whitespace-nowrap">Rp {{ number_format($totalIncome, 0, ',', '.') }}</span>
                                        </div>
                                        <div class="flex justify-between gap-2">
                                            <span>Pengeluaran:</span>
                                            <span class="font-medium whitespace-nowrap">Rp {{ number_format($totalExpense, 0, ',', '.') }}</span>
                                        </div>
                                        <div class="flex justify-between gap-2 pt-1 border-t border-red-200 mt-1">
                                            <span class="font-semibold">Kerugian:</span>
                                            <span class="font-semibold whitespace-nowrap">Rp {{ number_format(abs($totalProfit), 0, ',', '.') }} ({{ number_format(abs($profitPercentageRaw), 1) }}%)</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm">
            ...
            <div class="p-6">
                ...
                @if(isset($bestDay) && $bestDay)
                <div class="mt-4 p-3 bg-green-50 rounded-lg">
                    <p class="text-xs text-gray-500">Hari dengan Pendapatan Tertinggi</p>
                    <div class="flex justify-between items-center mt-1">
                        <span class="font-medium text-gray-800">{{ \Carbon\Carbon::parse($bestDay->date)->format('d/m/Y') }}</span>
                        <span class="font-semibold text-green-600">Rp {{ number_format($bestDay->amount, 0, ',', '.') }}</span>
                    </div>
                </div>
                @endif

                @if(isset($worstDay) && $worstDay)
                <div class="mt-2 p-3 bg-red-50 rounded-lg">
                    <p class="text-xs text-gray-500">Hari dengan Pendapatan Terendah</p>
                    <div class="flex justify-between items-center mt-1">
                        <span class="font-medium text-gray-800">{{ \Carbon\Carbon::parse($worstDay->date)->format('d/m/Y') }}</span>
                        <span class="font-semibold text-red-600">Rp {{ number_format($worstDay->amount, 0, ',', '.') }}</span>
                    </div>
                </div>
                @endif
                ...
                <div class="mt-6 pt-6 border-t">
                    ...
                    @if ($profitMargin >= 20)
                        <div class="text-sm text-green-600 bg-green-50 p-3 rounded-lg">
                            <div class="flex items-start">
                                <i data-lucide="check-circle" class="w-4 h-4 text-green-600 mr-2 shrink-0 mt-0.5"></i>
                                <span class="break-word">Bisnis dalam kondisi sangat sehat! Profit margin di atas 20% menunjukkan operasi yang efisien.</span>
                            </div>
                        </div>
                    @elseif($profitMargin >= 10)
                        <div class="text-sm text-blue-600 bg-blue-50 p-3 rounded-lg">
                            <div class="flex items-start">
                                <i data-lucide="alert-circle" class="w-4 h-4 text-blue-600 mr-2 shrink-0 mt-0.5"></i>
                                <span class="break-word">Bisnis dalam kondisi baik. Pertimbangkan untuk mengurangi biaya operasional untuk meningkatkan margin.</span>
                            </div>
                        </div>
                    @elseif($profitMargin >= 0)
                        <div class="text-sm text-amber-600 bg-amber-50 p-3 rounded-lg">
                            <div class="flex items-start">
                                <i data-lucide="alert-triangle" class="w-4 h-4 text-amber-600 mr-2 shrink-0 mt-0.5"></i>
                                <span class="break-word">Profit margin rendah. Evaluasi pengeluaran dan pertimbangkan penyesuaian harga.</span>
                            </div>
                        </div>
                    @else
                        <div class="text-sm text-red-600 bg-red-50 p-3 rounded-lg">
                            <div class="flex items-start">
                                ...
                                <span class="break-word">Bisnis mengalami kerugian. Segera evaluasi biaya operasional dan strategi harga.</span>
                            </div>
                        </div>
                    @endif
                </div>
...
function exportReport() {
    ...
    const url = '{{ route("reports.profit.pdf", ["startDate" => "PLACEHOLDER_START", "endDate" => "PLACEHOLDER_END"]) }}'
