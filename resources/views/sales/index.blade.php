...
@section('page-description', 'Kelola dan proses pesanan dari pelanggan')
        ...
        <div class="px-6 pt-6 border-b">
            <div class="flex space-x-4 overflow-x-auto pb-2">
                <a href="{{ route('sales.index') }}"
                    class="px-4 py-2 text-sm font-medium rounded-lg {{ !request('status') ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
                    Semua ({{ $totalTransactions }})
                </a>
                <a href="{{ route('sales.index', ['status' => 'pending']) }}"
                    class="px-4 py-2 text-sm font-medium rounded-lg {{ request('status') == 'pending' ? 'bg-yellow-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
                    Menunggu ({{ $pendingOrders }})
                </a>
                <a href="{{ route('sales.index', ['status' => 'accepted']) }}"
                    class="px-4 py-2 text-sm font-medium rounded-lg {{ request('status') == 'accepted' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
                    Diterima ({{ $acceptedOrders }})
                </a>
                <a href="{{ route('sales.index', ['status' => 'completed']) }}"
                    class="px-4 py-2 text-sm font-medium rounded-lg {{ request('status') == 'completed' ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
                    Selesai ({{ $completedOrders }})
                </a>
                <a href="{{ route('sales.index', ['status' => 'rejected']) }}"
                    class="px-4 py-2 text-sm font-medium rounded-lg {{ request('status') == 'rejected' ? 'bg-red-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
            ...
            <form method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="search" value="{{ request('search') }}"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500"
                        placeholder="No. Order atau nama kasir...">
                </div>
        ...
        <div class="p-6 border-b bg-gray-50">
            <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                ...
                <div class="text-center p-4 bg-white rounded-lg shadow-sm">
                    ...
                    <p class="text-2xl font-bold text-blue-600">Rp {{ number_format($totalSales ?? 0, 0, ',', '.') }}</p>
                </div>
                ...
                <div class="text-center p-4 bg-white rounded-lg shadow-sm">
                    ...
                    <p class="text-2xl font-bold text-green-600">
                        Rp
                        {{ number_format($totalTransactions > 0 ? $totalSales / $totalTransactions : 0, 0, ',', '.') }}
                    </p>
                </div>

                <div class="text-center p-4 bg-white rounded-lg shadow-sm">
                    <p class="text-sm text-gray-500">Belum Bayar</p>
                    <p class="text-2xl font-bold text-yellow-600">{{ $pendingPaymentCount ?? 0 }}</p>
                </div>
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <div
                                        class="h-10 w-10 shrink-0 bg-green-100 rounded-lg flex items-center justify-center">
                                    ...
                                    <div class="ml-4">
                                        ...
                                        @if ($sale->order)
                                            <div class="text-xs text-gray-500">{{ $sale->order->order_number }}</div>
                                        @endif
                                        @if ($sale->notes)
                                            <div class="text-xs text-gray-500">{{ Str::limit($sale->notes, 20) }}</div>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex space-x-2">
                                    @if ($sale->order && $sale->order->status === 'pending')
                                        <a href="{{ route('sales.process', $sale) }}"
                                            class="text-yellow-600 hover:text-yellow-900 p-1" title="Proses Pesanan">
                                            <i data-lucide="clipboard-list" class="w-4 h-4"></i>
                                        </a>
                                    @endif
                                    <a href="{{ route('sales.show', $sale) }}" class="text-blue-600 hover:text-blue-900 p-1"
                                        title="Detail">
                                        <i data-lucide="eye" class="w-4 h-4"></i>
                                    </a>
                                    @if ($sale->payment_status === 'paid')
                                        <a href="{{ route('sales.print', $sale) }}" target="_blank"
                                            class="text-green-600 hover:text-green-900 p-1" title="Cetak Struk">
                        ...
                        <tr>
                            <td colspan="8" class="px-6 py-12 text-center">
                                <div class="text-gray-400">
                                    ...
                                    <p class="text-sm mt-2">Pesanan dari pelanggan akan muncul di sini</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
